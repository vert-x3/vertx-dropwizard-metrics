...
A fairly simple API to retrieve metrics via the `link:../../yardoc/Vertx/Measured.html[Measured]`
interface which is implemented by various Vert.x components like `link:../../yardoc/Vertx/HttpServer.html[HttpServer]`,
`link:../../yardoc/Vertx/NetServer.html[NetServer]`, and even `link:../../yardoc/Vertx/Vertx.html[Vertx]` itself.
...
----
compile 'io.vertx:vertx-dropwizard-metrics:3.4.2-SNAPSHOT'
----
...
The `link:../dataobjects.html#DropwizardMetricsOptions#set_registry_name-instance_method[vertx.metrics.options.registryName]`
...
The `link:../dataobjects.html#DropwizardMetricsOptions#set_jmx_enabled-instance_method[vertx.metrics.options.jmxEnabled]` and
`link:../dataobjects.html#DropwizardMetricsOptions#set_jmx_domain-instance_method[vertx.metrics.options.jmxDomain]`
...
The `link:../dataobjects.html#DropwizardMetricsOptions#set_config_path-instance_method[vertx.metrics.options.configPath]`
...
The `link:../../yardoc/VertxDropwizard/MetricsService.html[MetricsService]` provides an API in front of the Dropwizard Registry to get
...
[source,ruby]
----
metrics = metricsService.get_metrics_snapshot(vertx)
...
metrics = metricsService.get_metrics_snapshot(eventBus)
metrics['handlers']
...
[source,ruby]
----
metricsNames = metricsService.metrics_names()
metricsNames.each do |metricsName|
  puts "Known metrics name #{metricsName}"
end
...
Once enabled, the `link:../../yardoc/VertxDropwizard/MetricsService.html[MetricsService]` allows to retrieve metrics snapshots from any
`link:../../yardoc/Vertx/Measured.html[Measured]` object which provides a map of the metric name to the data,
represented by a `link:unavailable[JsonObject]`. So for example if we were to print out all metrics
for a particular Vert.x instance:
[source,ruby]
----
require 'vertx-dropwizard/metrics_service'
metricsService = VertxDropwizard::MetricsService.create(vertx)
metrics = metricsService.get_metrics_snapshot(vertx)
puts metrics

----

NOTE: For details on the actual contents of the data (the actual metric) represented by the `link:unavailable[JsonObject]`
...
Since `link:../../yardoc/Vertx/HttpServer.html[HttpServer]` implements `link:../../yardoc/Vertx/Measured.html[Measured]`, you can easily grab all metrics
...
# set up server
metrics = metricsService.get_metrics_snapshot(server)
...
require 'vertx-dropwizard/metrics_service'
metricsService = VertxDropwizard::MetricsService.create(vertx)
metrics = metricsService.get_metrics_snapshot("vertx.eventbus.message")
...
configured by `link:../../vertx-core/dataobjects.html#HttpClientOptions#set_metrics_name-instance_method[metricsName]`.
...
configured by `link:../../vertx-core/dataobjects.html#NetClientOptions#set_metrics_name-instance_method[metricsName]`.
...
_vert.x-internal-blocking_. Named worker executor created with `link:../../yardoc/Vertx/WorkerExecutor.html[WorkerExecutor]` are exposed.
...
# Get the registry
registry = Java::ComCodahaleMetrics::SharedMetricRegistries.get_or_create("my-registry")
